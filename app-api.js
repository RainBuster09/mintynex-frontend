...
(function() {
  ...
  window.handlePhotoUpload = function(input, type) {
    ...
    reader.onload = function(e) {
      ...
      if (type === 'banner') {
        // Apply banner directly — skip crop modal (avoids canvas taint on mobile/Safari)
        const ban = document.querySelector('.prof-ban');
        if (ban) {
          ban.style.backgroundImage = 'url(' + dataUrl + ')';
          ban.style.backgroundSize = 'cover';
          ban.style.backgroundPosition = 'center';
          ban.style.backgroundRepeat = 'no-repeat';
          // Also remove default gradient so image shows
          ban.style.background = 'url(' + dataUrl + ') center/cover no-repeat';
        }
        if (typeof USER !== 'undefined') USER.bannerSrc = dataUrl;
        showToast('Banner updated! ✓', 'grn');
      } else {
        openCropModal(dataUrl, type);
      }
    };
...
async function populateGlobalCountryData() {
  ...
  // Init searchable dial dropdowns with flags
